...
Detects color on the pallete in the franka workspace.
...
Subscriptions:
  + image_raw Image - Gives the raw color image from the attached camera
  + camera_info CameraInfo - Gets the attached camera intrinsic information
...
class ImageListener(Node):
    ...
    def camera_info_cb(self, msg):
        """
        Get the camera instrinsics information to perform pixels and distance calculaiton.

        Args:
            msg: Getting camera information.
    ...
    def masking(self, image, tagpos):
        """
        Mask the palette location in the workspace.

        Args:
            image: Last image captured by the realsense.
    ...
    def colordetection(self, masked):
        """
        Detect the position of all the color on the palette.

        Args:
            masked: masked image from the masking function.
